...
> 本章命令中的节点名、IP、版本号、镜像加速地址和凭证均使用占位符（如 `<NODE_IP>`、`<VERSION>`、`<MIRROR_HOST>`）。执行前替换为实验环境实际值。不要将 kubeconfig、证书私钥提交到 Git 仓库。
...
# master1   Ready    control-plane   10d   v<VERSION>   192.168.10.11   Ubuntu 22.04
# master2   Ready    control-plane   10d   v<VERSION>   192.168.10.12   Ubuntu 22.04
# master3   Ready    control-plane   10d   v<VERSION>   192.168.10.13   Ubuntu 22.04
# worker1   Ready    <none>          10d   v<VERSION>   192.168.10.21   Ubuntu 22.04
# worker2   Ready    <none>          10d   v<VERSION>   192.168.10.22   Ubuntu 22.04
# worker3   Ready    <none>          10d   v<VERSION>   192.168.10.23   Ubuntu 22.04
```
...
# master1   Ready    control-plane   10d   v<VERSION>
# worker1   Ready    <none>          10d   v<VERSION>   cicd
# worker2   Ready    <none>          10d   v<VERSION>   infra
# worker3   Ready    <none>          10d   v<VERSION>   apps
```
...
```bash
# 在开发机执行（需要集群管理员权限）
# <VERSION> 替换为 local-path-provisioner 当前稳定版本号（见项目 Release 页）
kubectl apply -f \
  "https://raw.githubusercontent.com/rancher/local-path-provisioner/<VERSION>/deploy/local-path-storage.yaml"
...
```bash
# 在开发机执行（需要集群管理员权限）
# <VERSION> 替换为 Gateway API 当前稳定版本号（见 kubernetes-sigs/gateway-api Release 页）
kubectl apply -f \
...
```bash
# 在开发机执行（需要集群管理员权限）
# <VERSION> 替换为 Istio 当前稳定版本号（见 istio.io 官方下载说明）
